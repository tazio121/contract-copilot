...
def _choose_backend_base() -> str:
    """
    Prefer explicit BACKEND, then API_BASE, then a sane hosted default,
    otherwise fall back to local dev. Trailing slash is stripped.
    """
    # Detect Render environment hint (optional)
    _render_hint = os.getenv("RENDER_EXTERNAL_URL") or os.getenv("RENDER")
    # Order of preference
    cands = [
        os.getenv("BACKEND"),
        os.getenv("API_BASE"),
        # If you deploy with a known backend URL, keep it here as a friendly default:
        "https://contract-copilot-backend.onrender.com" if _render_hint else None,
        "http://127.0.0.1:8787",
    ]
    for c in cands:
        if c:
...
# If API_BASE wasn't explicitly set but BACKEND is, mirror it so pings use the same host.
if not os.getenv("API_BASE") and os.getenv("BACKEND"):
    API_BASE = os.getenv("BACKEND").rstrip("/")
...
# Config & Constants
# =============================================================================
# (kept your original default line)
BACKEND  = os.getenv("BACKEND", "http://127.0.0.1:8787")
# --- NEW: non-destructive override that only *adds* behavior -----------------
# Prefer explicit env, otherwise coalesce with API_BASE/Render-friendly default.
BACKEND = (os.getenv("BACKEND") or _choose_backend_base()).rstrip("/")
            ...
            with st.expander(head, expanded=expanded):
                ...
                if creasons:
                    ...
                    for rr in creasons[:3]:
                        st.write(f"• {rr}")
                    if len(creasons) > 3:
                        more_key = f"td_more_reasons_{i}"
                        if st.toggle(f"Show {len(creasons)-3} more reason(s)", key=more_key, value=False):
                            for rr in creasons[3:6]:
                                st.write(f"• {rr}")

                ctext = (c.get("text") or c.get("raw_text") or "").strip()
                if ctext:
                    txt_key = f"td_show_clause_text_{i}"
        ...
        for i, c in enumerate(clauses, start=1):
            ...
            with st.expander(head, expanded=bool(expanded)):
                ...
                if creasons:
                    ...
                    if len(creasons) > 3:
                        more_key = f"pdfd_more_reasons_{i}"
                        if st.toggle(f"Show {len(creasons)-3} more reason(s)", key=more_key, value=False):
                            for rr in creasons[3:6]: st.write(f"• {rr}")

                ctext = (c.get("text") or c.get("raw_text") or "").strip()
                if ctext:
                    txt_key = f"pdfd_show_clause_text_{i}"
                    if st.toggle("Show clause text", key=txt_key, value=False):
                        st.markdown(
                            f"<div style='border:1px solid #2a2f37;border-radius:8px;padding:8px 10px;margin:6px 0;background:#0f131a;white-space:pre-wrap;'>"
                            f"{ctext}</div>",
                ...
                names = sorted({(e.get("text") or "").strip() for e in ents if isinstance(e, dict) and e.get("text")})
                if names:
                    ent_key = f"pdfd_show_entities_{i}"
                    if st.toggle(f"Show entities ({len(names)})", key=ent_key, value=False):
                        st.markdown('<div class="chips">' + " ".join(f"<span>{n}</span>" for n in names[:40]) + "</div>", unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
...
# =============================================================================
    ...
    # --- robust prefs bootstrap (handles None) --------------------------------
    _loaded_prefs = st.session_state.get("prefs") or (_prefs() or {})
    if not isinstance(_loaded_prefs, dict):
        _loaded_prefs = {}
    default_prefs = {
        "display_name":  "New Pilot",
        "default_mode":  "Quick",
    ...
    prefs = {**default_prefs, **_loaded_prefs}
